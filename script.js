document.addEventListener('DOMContentLoaded', () => {
    ...
    // Split words into letters and wrap them in spans
    words.forEach((word) => {
    ...
    let changeText = () => {
        ...
        // Animate the current word letters out
        ...
        // Reset next word opacity to 0 before animating in
        ...
        // Set opacity of the next word to 1 after the animation starts
        setTimeout(() => {
...
window.addEventListener("scroll", function () { header.classList.toggle("sticky", window.scrollY > 50)
})
    ...
    const observer = new IntersectionObserver((entries)=>{ 
        ...
        document.querySelectorAll('.toggle-btn').forEach(button => {
            button.addEventListener('click', function(event) {
                event.preventDefault();
                const serviceText = this.closest('.service-box').querySelector('.service-text');
                const isExpanded = serviceText.classList.contains('expanded');
